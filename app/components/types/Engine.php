...
class Engine extends \infinite\base\Engine {
	...
	public function isReady() {
		...
		foreach ($this->_types as $type) {
			...
			if (!isset($this->tableRegistry[$type->module->shortName])) {
				return false;
			}
			if ($this->tableRegistry[$type->module->shortName]->system_version < $type->module->version) {
				return false;
			}
		}
		return true;
	...
	public function prepareTypes() {
		foreach ($this->_types as $type) {
			...
			$this->registerObjectType($mod);

			if (!isset($this->tableRegistry[$shortName])) {
				throw new Exception("Unable to initialize module $shortName");
			}

			if ($this->tableRegistry[$shortName]->system_version < $mod->version) {
				$oldVersion = $this->_tableRegistry[$shortName]->system_version;
				$this->_tableRegistry[$shortName]->system_version = $mod->version;
				if (!$mod->upgrade($oldVersion) OR !$this->_tableRegistry[$shortName]->save()) {
					throw new Exception("Unable to upgrade module $shortName to {$mod->version} from {$oldVersion}");
				}
			}
		}
		return true;
	...
	public function registerObjectType($mod) {
		$shortName = $mod->shortName;

		if (!isset($this->tableRegistry[$shortName])) {
			$this->_tableRegistry[$shortName] = new ObjectType;
			$this->_tableRegistry[$shortName]->name = $shortName;
			$this->_tableRegistry[$shortName]->system_version = $mod->version;
			if (!$mod->setup() || !$this->_tableRegistry[$shortName]->save()) {
